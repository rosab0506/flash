...
func sanitizeDefaultValue(defaultVal string, colType string) string {
	...
	validFunctions := []string{
		...
		"GEN_RANDOM_UUID()", "UUID_GENERATE_V4()", "TRUE", "FALSE",
	}
	...
	numericTypes := []string{"INT", "SERIAL", "DECIMAL", "NUMERIC", "FLOAT", "DOUBLE", "REAL"}
...
func (s *Service) generateCreateTable(change *SchemaChange) string {
	...
	if change.TableDef != nil {
		tableName = change.TableDef.Name
...
func (s *Service) getChangeDescription(change *SchemaChange) string {
	switch change.Type {
	case "add_column":
		return fmt.Sprintf("Add column '%s' to table '%s'", change.Column.Name, change.Table)
	case "drop_column":
		return fmt.Sprintf("Drop column '%s' from table '%s'", change.Column.Name, change.Table)
	case "modify_column":
		return fmt.Sprintf("Modify column '%s' in table '%s'", change.Column.Name, change.Table)
	case "add_table", "create_table":
		tableName := change.Table
		if change.TableDef != nil {
			tableName = change.TableDef.Name
