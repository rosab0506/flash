...
func (g *Generator) generateDatabase(queries []*parser.Query) error {
	...
	if len(filesList) == 1 {
		...
		for _, baseName := range filesList {
			...
			w.WriteString(fmt.Sprintf("        %s = %sQueries(db)\n", instanceName, className))

			w.WriteString(fmt.Sprintf("        # Copy methods from %s\n", className))
			w.WriteString(fmt.Sprintf("        for method_name in dir(%s):\n", instanceName))
			w.WriteString("            if not method_name.startswith('_') and callable(getattr(" + instanceName + ", method_name)):\n")
			w.WriteString("                setattr(self, method_name, getattr(" + instanceName + ", method_name))\n")
			w.WriteString("\n")
		}
	}
