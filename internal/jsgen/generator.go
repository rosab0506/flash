...
func (g *Generator) generateQueries(queries []*Query) error {
	...
	dts.WriteString("// Code generated by Graft. DO NOT EDIT.\n")
	dts.WriteString("export { Queries } from './index';\n")

	dtsPath := filepath.Join(g.Config.Gen.JS.Out, "queries.d.ts")
...
func (g *Generator) generatePostgreSQLExecution(w *strings.Builder, paramNames []string, hasColumns bool, cmd string, isSingleColumn bool, columns []*QueryColumn, isHotQuery bool) {
	...
	if hasColumns {
		...
		// No columns detected - could be INSERT/UPDATE/DELETE or failed column parsing
		// For :one and :many commands, return rows (these are SELECT queries)
		// For :exec, return rowCount
		switch cmd {
		case ":one":
			w.WriteString("    return r.rows[0] || null;\n")
		case ":many":
...
func (g *Generator) mapSQLTypeToJS(sqlType string) string {
	...
	switch {
	case contains(sqlTypeLower, "int"), contains(sqlTypeLower, "serial"):
		return "number"
	case contains(sqlTypeLower, "numeric"), contains(sqlTypeLower, "decimal"), contains(sqlTypeLower, "float"), contains(sqlTypeLower, "double"), contains(sqlTypeLower, "real"):
		return "number"
	case contains(sqlTypeLower, "bool"):
		return "boolean"
	case contains(sqlTypeLower, "json"):
		return "Object"
	case contains(sqlTypeLower, "timestamp"), contains(sqlTypeLower, "date"), contains(sqlTypeLower, "time"):
		return "Date"
	case contains(sqlTypeLower, "bytea"), contains(sqlTypeLower, "blob"):
		return "Uint8Array"
	default:
		return "string"
	}
